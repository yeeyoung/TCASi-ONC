...
Exp1 = data.ExpVdd10Iin100Cmem4pt7Cfb10_;
...
hold on;
plot(Exp1(1:50:end,1),Exp1(1:50:end,3),Marker_list(1),'color',C{2},'MarkerSize',10);
exper=Exp1(1:end,3);
error = immse(Vout(1:2000,2),exper(1:2000));
disp(['error = ',num2str(error)]);
error1 = error/immse(zeros(2000,1),exper(1:2000));
disp(['error1 = ',num2str(error1)]);
plot(Vmem(:,1),Vmem(:,2)-0*min(Vmem(:,2)),'-','LineWidth',2,'color',C{1});
plot(Exp1(1:50:end,1),Exp1(1:50:end,2),Marker_list(2),'color',C{1},'MarkerSize',10);
exper=Exp1(1:end,2);
error = immse(Vmem(1:2000,2),exper(1:2000));
disp(['error = ',num2str(error)]);
error1 = error/immse(zeros(2000,1),exper(1:2000));
disp(['error1 = ',num2str(error1)]);
hold off;
...
opt.FileName = 'figs/oAH_exp1.eps'; 
% create the plot
setPlotProp5(opt);
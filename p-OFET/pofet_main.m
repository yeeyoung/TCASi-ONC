%%%PSO optimization for POFET parameter extraction based on MARINOV model
...
data_G = data_G1(1:end,:); %351
result_G = result_G1(1:end,:);
data = flipud(data_G);
result = flipud(result_G);
...
fun = @(t) F(t);
for k = 1:size(data,1)
    q(k) = integral(fun,-data(size(data,1),paraindex),-data(size(data,1)-(k-1),paraindex));
end
q = q';
% Define the HVG function
HVG = q./(-flipud(result(:,paraindex)));
n = round (size(data,1)/20);
n = round(n);
dt_fit = -flipud(data(:,paraindex));
p1 = polyfit(dt_fit(19.2*n:20*n),HVG(19.2*n:20*n),1);
Threshold_Marinov = -(p1(2)/p1(1));
Threshold_Marinov = -Threshold_Marinov
...
% guarantee good fitting performance. Other tuned values are possible.
gama = -0.7553;     %Characteristic exponent (regressed from HVG function)
Vt = -3.5411;       %Threshold voltage (regressed from HVG function)
%Parameters for IDVG
Vds_G = -[20];      %Drain voltage for IDVG
Vgs_G = data_G;     %Gate voltage for IDVG
ID_ex_G = result_G(:,paraindex);
gs_G = size(Vgs_G,1);
ds_G = size(Vds_G,2);
...
% Set optimization lower and upper bounds for model parameters
nVar=7;                % Number of Non-intrinsic Variables to be optimized
VarSize=[1 nVar];      % Size of Decision Variables Matrix
% The following feasible parameter ranges can be relaxed during the optimization
VssMin= 1.203;         % Lower Bound of Vss
VssMax= 1.20301;       % Upper Bound of Vss
VbbMin= 0.5801;        % Lower Bound of Vbb
VbbMax= 0.580101;      % Upper Bound of Vbb
mu0Min = 0.613300;   % Lower Bound of gama
mu0Max = 0.613301;     % Upper Bound of gama
landaMin = 0.002387;   % Lower Bound of lambda
landaMax = 0.00238701; % Upper Bound of lambda
aaaMin = 3.17;         % Lower Bound of a in Double-Exponential;
aaaMax = 3.1701;       % Upper Bound of a in Double-Exponential;
bbbMin = -0.574601;    % Lower Bound of b (Combined with thermal voltage);
bbbMax = -0.5746;      % Upper Bound of b (Combined with thermal voltage);
cccMin = -23.0701;     % Lower Bound of c in Double-Exponential;
cccMax = -23.07;       % Upper Bound of c in Double-Exponential;
%% PSO Parameters
MaxIt=500;       % Maximum Number of Iterations
nPop=200;        % Population Size (Swarm Size)
% PSO Parameters (commented for default values)
% w=1;           % Inertia Weight
...
BestSol = GlobalBest;
%% Results (Optimized results of intrinsic parameters)
...
ylabel('abs(I_{D,p}) (A)','FontSize',18,'FontName','Times New Roman');
...
opt.FileName = 'figs/IDVG_POFET.eps'; 
% create the plot
setPlotProp5(opt);
...
ylabel('I_{D,p} (A)','FontSize',18,'FontName','Times New Roman');
xlabel('V_{DS} (V)','FontSize',18,'FontName','Times New Roman')
legend('Simulated','Measured','Location','northwest','LineWidth',0.5);
opt1 = [];
opt1.HoldLines = 1;
opt1.XLim = [-20, 0.1]; % [min, max]
opt1.YLim = [-6e-7, 0]; % [min, max]
...
opt1.FontSize = 18;
opt1.FileName = 'figs/IDVD_POFET.eps'; 
% create the plot
setPlotProp5(opt1);

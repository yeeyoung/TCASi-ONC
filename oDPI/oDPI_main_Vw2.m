...
data1 = data.EXPVtau8Vw0Vth0Csyn4dot7_;
data2 = data.EXPVtau8Vw5Vth0Csyn4dot7_;
data3 = data.EXPVtau8Vw10Vth0Csyn4dot7_;
...
Marker_list = ['o','s','^'];
Vw_list = [0,5,10]; Vthr = 0; Vdd = 10; Vtau = 8; 
Csyn = 4.7; Cgd_list = [0.8,0.8,0.8];% nF
...
hm = [];
hi = [];
time_sft = [46.751-5.049,51.1-5.049,12.1-5.049];
ax1 = axes();
 
for i = 1:3
    Cgd = Cgd_list(i);
    Vw = Vw_list(i); %Input = Input_list{i};
    out = sim('oDPI_exc.slx');
    Isyn = out.Isyn;
    Input = out.Input;
    hs = plot3(ax1,Isyn(:,1),Vw*ones(size(Isyn(:,1))),Isyn(:,2),'-','LineWidth',2,'color',C{2});
    view(3);
    hold on; 
    he = plot3(ax1,Exp1{i}(1+time_sft(i)*1000:200:end,1)-time_sft(i),Vw*ones(size(Exp1{i}(1+time_sft(i)*1000:200:end,1)-time_sft(i))),Exp1{i}(1+time_sft(i)*1000:200:end,2),Marker_list(1),'color',C{1},'MarkerSize',10);
...
ylabel(ax1,'V_{w} (V)','FontSize',18,'FontName','Times New Roman');
xlim(ax1,[0,30]); ylim(ax1,[0,10]); zlim(ax1,[0,4e-7]);
ax2 = axes('Position', ax1.Position);
for i = 1:3
    Vw = Vw_list(i);
    hi = plot3(ax2,Input(:,1),Vw*ones(size(Input(:,1))),Input(:,2),'-','LineWidth',2,'color','k');
    hold on;
end
...
text(ax1,max(xlim(ax1))*1.17, min(ylim(ax1))*1.17, ...
...
% kp = (gama_p+2)/Vss_p
% tau = Csyn*1e-9/(kp*I0p*exp(kp*(10-Vtau)))
...
Marker_list = ['o','s','^'];
Vw = 15;
Vdd = 20; Vtau_list = [13,14,15]; Csyn = 10; % nF
...
for i = 1:3
    ...
    hold on; 
    if (i==3)
        ylabel('I_{Syn} (A)','FontSize',18,'FontName','Times New Roman'); ylim([0.8e-10,1.2e-10]);
...
legend({'V_{\tau} = 13 V','V_{\tau} = 14 V','V_{\tau} = 15 V','PSP'},'FontSize',14,'FontName','Times New Roman');
...
opt.FileName = 'figs/oLDI_Vtau.eps'; 
% create the plot
setPlotProp5(opt);

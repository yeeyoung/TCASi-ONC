...
Cload = 0.01;
Vw = 5; Vthr = 5; Csyn = 1; Cgd = 0.8; Vtau = 8; % nF
Vw_exc = 0;
Vw_inh = -5; 
Vthr_inh = -8; %5
Vtau_inh = -8;%-5
...
opt.FileName = 'figs/circuit3_1.eps'; 
% create the plot
setPlotProp5(opt);
...
power_inh_DPI = out1.power_inh_DPI_Mtau(:,2)+out1.power_inh_DPI_Mthr(:,2)+out1.power_inh_DPI_Mw(:,2)+out1.power_inh_DPI_Mpre(:,2)+out1.power_inh_DPI_Min(:,2)+out1.power_inh_DPI_Msyn(:,2);
power_inh_DPI_mean = mean(power_inh_DPI)
...
ax = gca();
yyaxis left;
plot(out1.pd_AH_neuron(:,1),power_DPI*1e9,'-','LineWidth',2,'color',C{2});
hold off;
xlabel('Time (s)'); ylabel('Power Dissipation (nW)'); 
xlim([0,50]);
box on;
yyaxis right;
plot(Iinput1(:,1),Iinput1(:,2)-0*min(Iinput1(:,2)),'-','LineWidth',3,'color',C{5});
...
ax.YAxis(2).Color = 'k';
box on;
legend({'PD (Exc. oDPI)','I_{in1}','I_{in2}'},'FontSize',14,'FontName','Times New Roman');
...
opt.FontSize = 18;
% Save? comment the following line if you do not want to save
opt.FileName = 'figs/circuit3_1_eoDPI.eps'; 
% create the plot
setPlotProp5(opt);
...
ax = gca();
yyaxis left;
plot(out1.pd_AH_neuron(:,1),power_inh_DPI*1e9,'-','LineWidth',2,'color',C{2});
hold off;
xlabel('Time (s)'); ylabel('Power Dissipation (nW)'); 
xlim([0,50]);
box on;
...
ax.YAxis(2).Color = 'k';
box on;
legend({'PD (Inh. oDPI)','I_{in1}','I_{in2}'},'FontSize',14,'FontName','Times New Roman');
...
opt.FontSize = 18;
% Save? comment the following line if you do not want to save
opt.FileName = 'figs/circuit3_1_ioDPI.eps'; 
% create the plot
setPlotProp5(opt);
...
ax = gca();
yyaxis left;
plot(out1.pd_AH_neuron(:,1),power_AH*1e9,'-','LineWidth',2,'color',C{2});
hold off;
xlabel('Time (s)'); ylabel('Power Dissipation (nW)');
ylim([0,6000]); 
xlim([0,50]);
...
ax.YAxis(2).Color = 'k';
box on;
legend({'PD (oAH)','I_{in1}','I_{in2}'},'FontSize',14,'FontName','Times New Roman');
...
opt.FontSize = 18;
% Save? comment the following line if you do not want to save
opt.FileName = 'figs/circuit3_1_ah.eps'; 
% create the plot
setPlotProp5(opt);


Vw=5; Vw_inh=-5;
Iin1 = 200; Iin2 = 300;% nA
out2 = sim('SNN_circuit33.slx');
Vout = out2.Vout_left;
Vmem = out2.Vmem_left;
...
opt.FontSize = 18;
% Save? comment the following line if you do not want to save
opt.FileName = 'figs/circuit3_2.eps'; 
% create the plot
setPlotProp5(opt);
...
opt.FontSize = 18;
% Save? comment the following line if you do not want to save
opt.FileName = 'figs/circuit3_3.eps'; 
% create the plot
setPlotProp5(opt);
